...
Ce point sera discuté dans la \cref{sec:AM-FM-PM} suivante.
 \\
D'autre part, cela suggère que la phase géométrique doit avoir une certaine robustesse au bruit. \thoughts{J'ai un doute la dessus :} Pour argumenter ce point, rappelons que le bruit d'un signal se trouve généralement dans les hautes fréquences de ce dernier.
...
La phase géométrique est séparé en deux termes. Le premier, en arctangente, correspond à l'angle former par les vecteurs portant la polarisation instantanée, c'est-à-dire les vecteurs :
\begin{align*}
