...
\textbf{\textsc{\color{red} FAIT LE LIEN AVEC BERRY, \href{https://royalsocietypublishing.org/doi/10.1098/rspa.1984.0023}{ICI}}}
...
Dans cette décomposition, la composante en $\Id$ est indépendante de $\x$ et peut donc être ignorée (idem pour le facteur $\nicefrac{1}{2}$). Cela ne laisse qu'un vecteur (normé) de dimension 3 dont $2\theta$ et $2\chi$ correspondent aux coordonnées polaires conformément à la \Cref{fig:sphere2poincare} ci-contre.
\\ 
\\
...
\thoughts{Pas encore trop au point...}
...
Pour motiver d'autant plus cette décomposition, la projection $\rho : \x \lr \congu{\x}\, \transp{\x} / \|\x\|^2$, qui s'est avérée fort instructive, peut être vue comme une projection sur l'espace complexe en cela qu'elles sont toutes deux invariantes par transformation de jauge\footnote{\itshape
