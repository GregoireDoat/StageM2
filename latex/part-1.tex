...
\subsubsection{Lien entre les projections (/!$\backslash$ JE CONGUE PAS PAREIL)}
...
les expressions :
\begin{align*}
	2\rho_{P}(\theta, \chi) &= id + \sin(2\theta) \cos(2\chi) \sigma_1 - \sin (2\chi) \sigma_2 + \cos(2\theta) \cos(2\chi) \sigma_3 \\ 
	2\rho_{B}(\alpha, \beta) &= id + \cos(\alpha) \sin(\beta) \sigma_1 + \sin(\alpha) \sin(\beta) \sigma_2 + \cos (\beta) \sigma_3
\end{align*}
\skipl
...
\\
Sinon, avec les changements $\ 2\theta = \alpha\ $ et $\ 2\chi = \nicefrac{\pi}{2} - \beta$, on a cette fois :
\[\begin{pmatrix}
	\sin(2\theta) \cos(2\chi) \\ -\sin (2\chi) \\ \cos(2\theta) \cos(2\chi)
\end{pmatrix} = \begin{pmatrix}
...
\end{pmatrix}\]
\\
Finalement, dans la sphère la transformation se fait via :
\[w_P(2\theta, 2\chi) = \begin{pmatrix}
	1 & 0 & 0 \\ 0 & 0 & -1 \\ 0 & 1 & 0
\end{pmatrix} w_B\big( (\nicefrac{\pi}{2}-\theta ), \big(\nicefrac{\pi}{2}-\chi) \big)\]
\\

Encore une autre formule. Cette fois, en partant de \eqref{eq:bloch2poinca} :
\begin{align*}
...
\\
et les phases totale et dynamique se transforme suivant :
\begin{align*}
	...
	&= \psi(t) + \alpha(t)/2 - \big( \psi(t_0) + \alpha(t_0)/2 \big) - \int_{t_0}^t \alpha'(s)/2\sin \big( -2\beta(s)/2 \big) ds \\
	&= \psi(t) - \psi(t_0) + \frac{\alpha(t) - \alpha(t_0)}{2} + \frac{1}{2} \int_{t_0}^t \alpha'(s)\sin \beta(s) ds
\end{align*}
\\
Mais dans le même temps, si on déroule le calcul de la phase dynamique de $\x_B$, on tombe cette fois sur :
\begin{align*}
	\phased\big(\x_B(\psi, \alpha, \beta) \big) &= \psi(t) - \psi(t_0) +  \int_{t_0}^t \alpha'(s) \frac{1 - \cos\beta(s)}{2}ds \\
	&= \psi(t) - \psi(t_0) + \frac{1}{2}\big(\alpha(t) - \alpha(t_0) \big)- \frac{1}{2} \int_{t_0}^t \alpha'(s) \cos\beta(s) ds 
\end{align*}
Auquel cas :
\begin{align*}
	...
	&= \varphi(t) - \varphi(t_0) - \big( \theta(t_0) - \theta(t) \big) + \int_{t_0}^t \theta' (1 - \cos2\chi)ds
\end{align*}
