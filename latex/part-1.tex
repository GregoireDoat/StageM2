...
Pour rentre compte de la pertinence de cette expression, commençons par noter qu'il exsite une autre façon standard de définir calculer la phase d'un signal via la \emph{phase totale} :
\begin{equation}\label{eq:phase_t}
	\phaset(\bf{x}, t_0, t) \defeq \arg\big\langle \bf{x}(t), \bf{x}(t_0)\big\rangle
\end{equation}
Il n'est pas clair, dans un cadre générale comment pourquoi cela s'interprète bien comme une phase et c'est encore pire lorsque l'on explicite ca valeur :
\[\phaset(\bf{x},t_0, t) = \arg \sum{i=1}^n a_i(t)a_i(t_0)e^{i\phi(t)-\phi(t_0)}\]


et on reviendra dans la \namecref{part:phase_geo} suivante sur 
détaillons un cas particulier. Supposons que le signal $\bf{x}$ soit cyclique à une phase près. C'est-à-dire qu'entre les deux instant $t_0$ et $t$ donnés, l'on ait la relation :
...
\[\arg\big\langle \bf{x}(t), \bf{x}(t_0)\big\rangle = \theta = -\Im m\int_{t_0}^t \frac{\big\langle \bf{x}(s) , \dot{\bf{x}}(s) \big\rangle}{\|\bf{x}(s)\|^2} ds\]
\skipl

Or, si c'est vrai dans le cas univarié (\ie~$n=1$), dès que $n\geq 2$, ce n'est plus le cas. Cette erreur, cette nouvelle phase ne peut pas être 
