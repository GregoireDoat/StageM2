	...
	\begin{enumerate}[label=\arabic{enumi}.\arabic* --- ]
		...
		\begin{enumerate}[label=\arabic{enumi}.2.\arabic* --- ]
			...
			\begin{itemize} \normalfont
				...
				\item lien avec la projection $\x \longmapsto \x\x^\dagger$
...
Le ruban de Modiüs en est un exemple typique : il ne peut pas s'écrire comme le produit d'un cercle avec un segment $\S{1}\times [0,1]$ à cause de la façon dont il est construit. Mais localement, le ruban est tout à fait comparable (difféomorphe) à un tel produit (\cf~\cref{fig:fibration2modius}).
\begin{figure}[h]
	\begin{floatrow}
		\ffigbox{\caption{Fibration du ruban de Mobiüs.}
			\label{fig:fibration2modius}}{
			\includegraphics[width=0.45\textwidth]{fig/placeholder}
			}
		
		\ffigbox{\caption{Représentation d'un fibré principale}
...
Il exsite toutes sorte de variétés fibrées dès lors qu'elles sont munies de structure remarquable. Celle qui vont nous intéresser sont les variétés dites principales\footnote{\itshape
...
\begin{definition}[Variété fibrée principale] \label{def:VFP}
	...
	\begin{itemize}
		\item Il existe un groupe de Lie $G$ opérant à droite (ou à gauche) sur $P$ via une application différentiable :
		...
		\end{align*}
	\end{itemize}
	On dit alors que $B$ est la \emph{base} de la VFP, que $G$ est son \emph{groupe structural} et $pG$ est la \emph{fibre de $P$ passant par} $p$ et \emph{au dessus de} $\pi(p)\in B$. Le tout est notée $P(\phi, G, \pi, B)$ ou plus simplement $P(G,B)$.
\end{definition}
...
\begin{figure}[h]
	...
	\caption{Fibration du ruban de Mobiüs.}
	\label{fig:fibration2modius}
\end{figure}
\noindent
...
\begin{figure}[h]\centering 
	...
	\begin{tikzcd}[column sep=tiny]
		...
		& G 
	\end{tikzcd}
	\caption{Digrame surement osef des jeux de projections entre $P$ est les cartes locales}
\end{figure}
