...
\begin{definition}[Connexion sur VFP] \label{def:connexion2VFP}
	...
	\begin{itemize}
		...
		\end{equation}
		Que l'on notera plus simplement \thoughts{(\cf~annexe \ref{ann:2Lie})} :
		\begin{equation}
...
En admettant l'interprétation de la 1-forme de connexion comme fréquence instantanée, les discussions de première partie (\Cref{subsec:intro_phased}) suggèrent de choisir :
...
\\
\\
\thoughts{``Ok, je m'arrête ici. 
	
	A quelques détails près je pense que c'est très bien. Tu as fait la "connexion" entre description via les fibrés et notion de phase instantanée. Il faudra le dire explicitement (au debut de la partie ou dans l'intro ?)'' :\ COMMENT ?}
\\
...
Comme dans la première partie de ce mémoire, pour mieux comprendre le comportement de la phase géométrique, on se 
