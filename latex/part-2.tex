...
\subsection{$\S{n}\times\bf{\PC{n}}$ comme fibré principal} \label{subsec:VFP}
...
\subsubsection{Le fibré $\bf{\S{n}\big(\U{1},\PC{n}\big)}$}
...
\subsubsection{Choix de connexion sur $\bf{\S{n}\big(\U{1},\PC{n}\big)}$}\label{subsec:conn2SUPC}
