...
\begin{definition}[1-forme de connexion] \label{def:1-form2conn}
	...
	\begin{align} \label{eq:1-form2conn}
		\forall \bf{v}\in \mathfrak{g},\ \conn_p(\bf{v}) &= \bf{v}  &  \hg[p]{P} &= \Ker(\conn_p)
	\end{align}
	\skipl
...
Soit $\gamma$ une trajectoire de $P$ de projection $\rho_\gamma$ et de relèvement horizontale $\Tilde{\gamma}$, avec localement :
\begin{align*}
	\gamma(t) &= h_i\big( g_i(t), \rho_\gamma \big)  &  \Tilde{\gamma}(t) &= h_i\big( \Tilde{g}_i(t), \rho_\gamma \big)
\end{align*}
\\
Par définition, on a $\Tilde{g}$ suit l'équation \eqref{eq:EDP2hori_lift} et le passage de $\gamma$ à $\Tilde{\gamma}$ se fait suivant :
\[\gamma(t) = h_i\big( g_i, \rho_\gamma \big) = h_i\big( \Tilde{g}_i(t), \rho_\gamma \big) \cdot g_i \Tilde{g}^{-1}_i = \Tilde{\gamma} \cdot g_i \Tilde{g}^{-1}_i\]
\\
\\
\begin{itemize}
...
Il s'avère que ce choix est doublement justifié par le fait que c'est la connexion induite par la métrique de $\S{n}$.
