...
\begin{wrapfigure}{r}{0.3\textwidth}
	...
	\end{tikzcd}
	\caption{Diagramme de passage de $f$ à $f_*$ et/ou $f^*$.}
...
\subsubsection{\wip $\bf{\PC{n}}$, une variété complexe} \label{subsec:PC^n_variet}
...
Juste avant d'y venir, une remarque : ce lien entre la métrique de $\S{n}$ et celle de $\PC{n}$ s’avérera très utile puisqu'il permet de faire tout les calculs de norme dans $\S{n}$. Par exemple, si $\rho$ est une courbe de $\PC{n}$ avec $\gamma$ un de ses relèvements, alors sa longueur s'écrit :
\[\int \sqrt{g_{\rho} \big(\dot{\rho}, \dot{\rho}\big) } d\rho = \int \sqrt{\big\langle \dot{\gamma}_H, \dot{\gamma}_H \big\rangle} d\gamma\]
où $\bf{v}_H$ est la composante horizontale de $\bf{v}\in\tg{\S{n}}$.
Sachant que $\S{n}$ est récrite de façon extrinsèque et que sa métrique est induite par celle de $\C^{n+1}$, cela simplifie les calculs théoriques et pratiques.
\\
...
Notamment dans le cadre quantique, la phase géométrique est connue pour avoir deux interprétations géométriques \cite{bohm_geometric_2003, chruscinski_geometric_2004,faure_introduction_2022} : soit comme conséquence d'un transport parallèle sur $\S{n}$ soit comme une mesure de l'air entouré par le signal projeté sur $\PC{n}$. Ici ses résultats seront redémontrés (avec les détails en annexes) dans le notre dans notre cadre -- plus général --- et réinterpréter en terme de signal.

Pour se faire, sera d'abord traité le cas particulier des signaux cycliques (\Cref{subsec:phase_g2cycl}). Les résultats seront ensuite généralisé au cas plus générale (\Cref{subsec:phase_g2geode}) via l'étude des géodésiques de $\PC{n}$ avant, enfin, de faire montrer le lien entre phase géométrique et les aires sur $\PC{n}$ (\Cref{subsec:phase_g2aire}).
\skipl
...
Dans toute la suite un signal $\gamma$ se $\S{n}$ sera dit cyclique s'il entre les instants $t_0$ et $t$, $\gamma$ retourne dans la même fibre :
...
\begin{equation}\label{eq:phase_g-cycl}
	\begin{aligned}
		...
		&= t\big( \Tilde{\gamma}(t_0), \Tilde{\gamma}(t) \big)  &  \text{car }\gamma(t_0) &= \Tilde{\gamma}(t_0)
	\end{aligned}
\end{equation}
\\
...
, \ie~$\phaseg$ peut prendre absolument n'importe quelle valeur (alors même que l'on est toujours dans le cas particulier des signaux cycliques). Ceci n'est donc pas très instructif mais ce n'est pas la seule façon de voir les choses.
\\



\subsubsection{\todo  ... du point de vue de la métrique} \label{subsec:phase_g2geode}

Une autre façon de voir la phase géométrique de considérer un troisième relèvement de $C_\gamma$, $\eta$, qui soit un lacet de $\S{n}$. C'est-à-dire telle que $\eta(t) = \eta(t_0) = \gamma(t_0)$.
\\
Dans ce cas, la phase totale de $\eta$ est nulle et :
\[\phaseg(\gamma) = \phaseg(\eta) = -\phased(\eta)\]
\\
Soit encore, via \eqref{eq:phased_vgeodiff} :
\[\phaseg(\gamma) = -\frac{1}{i}\int_{t_0}^t \conn_{\eta(s)} \big( \dot{\eta}(s) \big) ds = i\oint_\eta \conn\]
\\
Le résultat obtenu est l'intégrale d'une forme linéaire le long d'un lacet, ce à quoi le théorème de Stokes s'applique
 : étant donnée une surface $\Sigma$ de bord $\eta$, on a :
\[\phaseg(\gamma) = i\oint_\eta \conn = i\iint_\Sigma d \conn\]
\\
Où la dérivée extérieure de $\conn$ n'est autre que la forme de Kähler de $\PC{n}$ ramené sur $\S{n}$ (\cf~annexe \ref{ann:stokes} pour une démonstration) :
\begin{equation} \label{eq:phaseg-aire2S}
	!! \phaseg(\gamma) = i\iint_\Sigma d \conn  = -i\iint_\Sigma \delta_{\mu \nu} dz^\mu \wedge d\congu{z}^\nu
\end{equation}
\\
Ce qui en ramené dans $\PC{n}$ donne :
\begin{equation} \label{eq:phaseg-aire2PC^n}
	!! \phaseg(\gamma) = -i\iint_\Sigma \delta_{\mu \nu} dz^\mu \wedge d\congu{z}^\nu = \iint_{\pi(\Sigma)} \Omega_{\mu\congu{\nu}} dw^\mu\wedge d\congu{w}^\nu 
\end{equation}
\\
Ainsi, la phase géométrique de toute courbe cyclique $\gamma$ est donnée par la demi-aire de la surface entourée par sa projection $\pi(\gamma)$ sur $\PC{n}$.

\begin{remarque}
	On pourrait alors se demander si $\phaseg$ ne pourrait pas, comme $\phased$, s'écrire comme l'intégrale d'une 1-forme sur $\PC{n}$.
	\\
...
\subsubsection{\todo ... dans le cas le plus générale} \label{subsec:phase_g2aire}
...
\subsection{\todo Calcul pratique de la phase géométrique}
...
\begin{itemize}
	...
	\item Il faut faire du calcul approximé d'aire sur $\PC{n}$. 2 pb : \begin{itemize}
		...
		\item  $\PC{n}$ est une variété quoitent : MEH
