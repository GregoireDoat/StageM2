...
\subsection{Construction de $\bf{\PC{n}}$} \label{subsec:construc_PC^n}

\subsubsection{Rappels de géométrie différentielle et notations}\label{subsec:rappel2geo_diff}

\begin{itemize}
	
	\item On fait des rappelles rapides, c'est surtout pour fixer les notations
	
	\item DEF : variété différentielles de classe $C^k$
	
	\item Espace et fibré tangent, espace des formes et leurs base respective
	
	\item NOTATION : on notera toujours les indices de coordo en lettres grèques ($\mu,\nu$) et les indices de cartes avec l'alphabet classique ($i,j,k$). 
...
\subsubsection{$\bf{\PC{n}}$ comme variété différentielle}\label{subsec:rappel2geo_diff}
...
\begin{proposition}
	...
	\begin{itemize}
		...
		\[\forall z \in \pi^{-1}(U_i),\ \forall e^{i\theta}\in\U{1},\  h_i(z,e^{i\theta}) = \frac{w^\mu}{\sqrt{1 + w^\nu \congu{w}_\nu}}e^{i\theta} \in \S{n}\qquad \text{ où les}\]
...
\subsection{Connexion sur VFP}\label{subsec:connexion2VFP}
...
\begin{itemize}
	...
	\item Un choix qui est doublement justifié puisque c'est également la 1-forme de connexion induite par la métrique sur $\S{n}$.
