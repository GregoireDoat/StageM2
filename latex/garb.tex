...
\begin{enumerate}[label=\Roman* --- ]
	...
	\item Introduction du phénomène pour les signaux 
	\begin{enumerate}[label=\arabic* --- ]
		
		\item Paramètres instantanées d'un signal complexe
		...
		\item Généralisation aux signaux analytique
		\begin{enumerate}[label=\arabic{enumi}.\arabic* --- ]
	...
	\item Description de la phase géométrique
	\begin{enumerate}[label=\arabic* --- ]
		
		\item  Cas des signaux pseudo-cycliques 
		\begin{enumerate}[label=\arabic{enumi}.\arabic* --- ]
			\item $\S{2n+1}$ vu comme VFP
			
			\item Interprétations des phases
			
			\item peut-être quelques mot sur l'``universal $\U{1}$ principal bundle''
		\end{enumerate}
		
		\item La phase géom dans l'espace projectif
		\begin{enumerate}[label=\arabic{enumi}.\arabic* --- ]
			\item Construction de $\PC{n}$ et de sa métrique
			
			\item Géodésique et lien avec le cas cyclique
			
			\item Birgmann, Bonnet-Gauss  \& Stokes
		\end{enumerate}
		
		\item Cas non-commutatif ?
		\begin{enumerate}[label=\arabic{enumi}.\arabic* --- ]
	...
	\item Cas des signaux AM-FM-PM
	\begin{enumerate}[label=\arabic* --- ]
		
		\item Bivarié \& trivarié
		
		\item Potentielle généralisation ?
		
		\item Comment calculer les $(\phi, \theta, \chi)$ ?
