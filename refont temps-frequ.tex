...
\section{Analyse temps-fréquence} \label{sec:temp-freq}

Dans toute cette \namecref{sec:temp-freq}, on considérera $x$ un signal complexe et on notera $\fou{x}$ ou $\Fou{x}$ sa transformée de Fourier (dont on supposera quelle existe, ici on supposera au moins $x\in L^2(\R, \C)$) :
\begin{align*}
...
\end{align*}
\\
On note $\densit$ (resp. $\densis$) la \emph{densité d'énergie} (resp. \emph{spectrale}) du signal $x$, définit par :
\begin{align}\label{eq:densi_dE}
	\densit\ &:\quad \begin{aligned}\R\ &\lr\quad \R^+ \\ t\ &\longmapsto\ \big|x(t)\big|^2 \end{aligned}  
	&
	\text{resp. }\ \densis\ &:\quad \begin{aligned}\R\ &\lr\quad \R^+ \\ \nu\ &\longmapsto\ \big|\fou{x}(\nu)\big|^2 \end{aligned}
\end{align}

Pour plus de détail sur l'interprétation de ses densités et les outils d'analyse temps-fréquence utile au propos du mémoire, voir l'annexe \ref{ann:temps-freq}.

%\section{Signal analytique, transformé de Hilberts, Analyse en temps-fréquence}
\subsection{Amplitude, phase et fréquence instantanée }\label{sec:temp-freq}
...
\'Etant donné un signal complexe $\ x:t\longmapsto x(t)\in\C $, son module et sa phase à l'instant $t$ peuvent être interpréter comme l'\emph{amplitude} et la \emph{phase instantanée} de $x$.
\\
...
Cela invite à définir $\phi'$ (resp.$\nicefrac{1}{2\pi}\phi'$) comme la l'\emph{impulsion} (resp. \emph{fréquence}) \emph{instantanée} du signal $x$. Définition qui est d'autant plus justifiée par la \namecref{prop.esp_freq} suivante :

\begin{proposition}\label{prop.esp_freq}
	Si $\densis$ admet une espérance et que $x$ est dérivable et que l'on note :
	\begin{align*}
		x(t) &= a(t) e^{i\phi(t)}  &  \text{resp. }\quad \fou{x}(\nu) &= \alpha(\nu) e^{i\psi(\nu)}
	\end{align*}
	alors on a l'égalité (cf. \cref{ann:temp-fraq} pour une démonstration) :
	\begin{align}\label{eq:esp_freq}
		\int_\R \nu \densis(\nu) d\nu &= \frac{1}{2\pi}\int_\R \phi'(t)\densit(t)dt
		  & & \text{ou encore}  &  
		\esp[\densis]{\nu} &= \frac{1}{2\pi} \esp[\densit]{\phi'}
	\end{align}
\end{proposition}
{\color{white}l} \\
\noindent 
Autrement dit, la moyenne temporelle de $\nicefrac{1}{2\pi} \phi'$ est égale à la fréquence moyenne du signal, ce qui renforce l'idée que c'est la ``bon choix'' de fréquence instantanée.
...
\subsection{Complément sur l'analyse temps-fréquence}\label{ann:temp-freq}

\subsubsection{Interprétation des densités et de leurs premiers moments}\label{subann:distrib_temp-freq}


Avec les notations introduites dans la \cref{sec:temp-freq}, la valeur $\densit(t)$ correspond à la puissance (énergie par unité de temps) déployée pour émettre le signal à l'instant $t$ et $\densis(\nu)$ à l'énergie associée à la fréquence $\nu$ sur tout le signal. 
\\
...
La transformée étant une isométrie de l'espace $L^2(\R,\C)$, l'\emph{énergie totale} du signal, elle, est indépendante de la représentation de ce dernier (temporelle ou fréquentielle) :
\begin{equation}\label{eq:parceval}
	E(x) \defeq {\|x\|_2}^2 = \int_\R \densit(t) dt = \int_\R \densis(\nu) d\nu = {\|\fou{x}\|_2}^2
\end{equation}

On rappelle que l'espérance des densités $\rho$ et $\varrho$, pour peu qu'elles existent, sont notées :
\begin{align*}
	\esp[\densit]{t} &\defeq \int_\R t \big|x(t)\big|^2 dt   &  \esp[\densis]{\nu} &\defeq \int_\R \nu \big|\fou{x}(\nu)\big|^2 d\nu
\end{align*}

Si un signal est localisé temporellement, alors l'espérance/moyenne $\esp[\densit]{t}$ donne une idée de l'instant d'émission du signal. Si \acontrario, le signal est localisé en fréquence, l'espérance  $\esp[\densis]{\nu}$ peut s'interpréter comme la fréquence ``dominante'' du le signal, ou plus généralement comme sa \emph{fréquence moyenne}. \\
...
\subsubsection{Formule de la fréquence instantanée}

Pour montrer la formule de la \cref{prop.esp_freq}, s'écrivent en fonction des dérivées $\fou{x}$ (resp. $x$) via ce qui Cohen \cite{cohen_time_1995} appelle les  :
...
En ce sens, la moyenne de $\phi'$ en temps est égale (à $2\pi$ près) à la fréquence moyenne de $x$.
