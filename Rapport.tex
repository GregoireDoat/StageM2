...
\`A noter tout de même, que la notion de fréquence instantanée est paradoxale en soit. Comme La fréquence ne peut pas être une information purement local
\\
...
\begin{definition}[valeur principale de Cauchy]\label{def:vp&Hilb}
	...
	\begin{equation}
		...
		\forall \varphi\in\mathcal{S}(\R),\qquad 
			...
			&= \int_{\R^+} \frac{\varphi(t) - \varphi(-t)}{t}dt
		\end{aligned}
	\end{equation}
...
Pour comprendre comment cette transformation ``sélectionne'' la fréquence instantanée, détaillons le cas où $x$ s'écrit comme somme de deux signaux pures (\cref{fig:exemple_tSA}) :
\[x(t) = \cos 2\pi\nu_1t + \cos 2\pi\nu_2t\]

\begin{figure}[h]\centering
	\includegraphics[width=0.48\textwidth]{fig/somme_signal-decompo.png}\hfill
	\includegraphics[width=0.48\textwidth]{fig/somme_signal-a_phi.png}
	\caption{Représentation graphique du signal $x$ (bleu). Sur l'image de gauche, avec signaux de fréquences pures (rouge et vert). Sur l'image de droite, avec son amplitude (orange) et sa phase instantanée (violet)}
	\label{fig:exemple_tSA}
\end{figure}
\noindent
On montre sans mal\footnote{\itshape
	$\fou{x}$ est donné par 4 Diracs, en ne gardant que ce qui sont non nul sur $\R^+$ on obtient le spectre de $\SA{x}$ et il reste plus qu'à inverser la transformée de Fourier.}
 que si $\nu_1\geq\nu_2$, alors la transformée en SA de $x$ s'écrit :
\[\SA{x} = \cos \left(2\pi\frac{\nu_1-\nu_2}{2}t\right) e^{2\pi\frac{\nu_1+\nu_2}{2}t}\]
\\
C'est donc la plus haute fréquence $\nu_1+\nu_2$ qui correspond à la fréquence instantanée du signal. Un résultats qui est rassurant puisque, moralement, cela assure qu'aucune fréquence ne s'est perdue dans l'amplitude.
\textsc{To Do} :
\begin{itemize}
	
	\item 
	
	\item Exemple pour dire que les hautes fréquences partes dans la phase (produit de cos). C'est super important ça et ça fait sens, si les hautes partaient dans l'amplitude, alors on pourrait avoir des signaux sans phase et uniquement à amplitude variables. Ce qui ne dit rien en somme.
