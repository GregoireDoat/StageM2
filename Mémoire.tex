...
La phase géométrique fait partie de ces concepts qui apparaissent régulièrement en physique, mais qui demande beaucoup de contexte pour être mis en évidence.
Pour l'introduire rapidement, la phase géométrique à l'instant $t$ d'un signal multivarié complexe (\ie~à valeur dans $\C^n$) $\x$ est donné par :
\[\phaseg(\x, t_0, t) = \arg\big\langle \x(t), \x(t_0)\big\rangle - \Im m\int_{t_0}^t \frac{\big\langle \dot{\x}(s) , \x(s) \big\rangle}{\|\x(s)\|^2} ds ds\]
...
La phase géométrique est un phénomène qui apparaît dans de nombreuses circonstances, en fonction desquelles elle peut changer de nom et de forme : phase Pancharatnam, de Berry, d'Aharonov-Anandan, d'Aharonov-Bohm, angle de Hannay, \etc
\\
...
L'objectif de ce mémoire est donc de réintroduit la phase géométrique dans le cadre du traitement du signal et de discuter de ses applications :
\\
\begin{itemize}
	\item Dans un premier temps (\cref{part:param_instant}), cette phase sera mise en évidence à travers des concepts d'analyse temps-fréquence, notamment la notion fréquence instantanée qui sera présente tout au long de l'écrit. 
	Suite à quoi elle sera explicitement calculée dans une cas particulier de signaux, déjà étudié par Le Bihan \etal~\cite{le_bihan_geometric_2024} : les signaux dit AM-FM-PM.
	...
	Des résultats, connus par ailleurs, sur la phase géométrique seront redémontrés avec ce formalisme purement géométrique et avec, les notions de fréquences instantanées et de phase géométrique seront reformuler et réinterpréter. 
	
	\item Enfin, dans une troisième partie, sera présenté un moyen de calculer la phase géométrique en pratique via l'invariant de Bargmann, tiré de \cite{rabei_bargmann_1999} et déjà repris par Le Bihan \etal~\cite{le_bihan_geometric_2024}. Seront ensuite discutées diverses applications \thoughts{et là sa dépend d'à quelle point j'ai le temps}.
\end{itemize}
