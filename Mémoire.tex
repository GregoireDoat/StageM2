...
\begin{itemize}
	...
	\item Ca a déjà étant fait en dimension 2 et un petit peu regarder en dimension 3 \cite{bihan_geometric_2024,le_bihan_modephysiques_2023}
...
\subsubsection{Interprétabilité de la transformée en SA}\label{subsec:Bedrisan&AM-FM}
...
\begin{itemize}
	\item D'une part, l'espérance de la fréquence instantanée (ici vu comme dérivée à $2\pi$ près de la phase\footnote{\itshape
		La pertinance de cette définition dans le cas multivarié sera discuté plus loin... \textbf{or is it ?}})
...
La seconde approche, fortement inspirée par les travaux de Lilly \& Olhede  \cite{lilly_analysis_2012}, se base sur l'idée de séparation haute-basse fréquence du signal $\bf{x}$. Pour cela, l'on commence par faire apparaître la phase $\phi$ --- pour l'instant inconnue --- en écrivant $\bf{x}$ sous la forme :
...
Pour s'en assurer on demande, à l'inverse, que les basses fréquences du signal soient données par $\Tilde{\bf{x}}$ en limitant ces variations. Concrètement, $\phi$ doit être choisie de sorte à minimiser la dérivée $\Tilde{\bf{x}}'$ :
\[\forall t\in\R,\qquad \phi(t) = \argmin{\theta(t)}{\big\|\Tilde{\bf{x}}'(t)\big\|_2}^2 = \argmin{\theta(t)}{\Big\|e^{-i\theta(t)}\big(\bf{x}'(t) - i\theta(t)'\bf{x}(t)\big)\Big\|_2}^2 = \argmin{\theta(t)}{\big\|\bf{x}'(t) - i\theta'(t)\bf{x}(t)\big\|_2}^2\]
\\
La contrainte ne dépendant que de la dérivée $\theta'$, on se ramène à :
\[\min_{\theta(t)}{\|\Tilde{\bf{x}}'(t)\|_2}^2 = \min_{\theta'(t)}{\big\|\bf{x}'(t) - \theta'(t) \bf{x}(t)\big\|_2}^2\]
\\
...
\[-\Im m\int \frac{\big\langle \bf{x}(s) , \bf{x}'(s) \big\rangle}{\|\bf{x}(s)\|^2} ds = \int \sum_{i=1}^n a_i(s)^2 \phi_i'(s) = \sum_{i=1}^n \int a_i(s)^2 \phi_i'(s)ds\]
Il semblerait alors que $ \frac{\Im m\langle  \bf{x}' ,  \bf{x}\rangle}{{\| \bf{x}\|_2}^2}$ soit un bon choix de phase instantanée. \`A noter que cette phase n'est définie qu'à une constante d'intégration près, autrement dit, à un choix de phase initiale près.
\\



\subsubsection{Apparition de la phase géométrique --- MEH}\label{subsub:intro_phaseg}

Pour rentre compte de la pertinence de cette expression, détaillons un cas particulier. Supposons que le signal $\bf{x}$ soit cyclique à une phase près. C'est-à-dire qu'entre les deux instant $t_0$ et $t$ l'on ait la relation :
\[\exists \theta\in\R\ |\ \bf{x}(t) = e^{i\theta} \bf{x}(t_0)\]
\\
Dès lors, la phase instantanée à l'instant $t$ devrait être correspondre à $\theta$ et on peut l'écrire en fonction de $\bf{x}$ :
\[\arg\big\langle \bf{x}(t), \bf{x}(t_0)\big\rangle = \arg\big\langle e^{i\theta} \bf{x}(t_0), \bf{x}(t_0)\big\rangle = \theta\]
\\
On pourrait alors s'attendre à avoir l'égalité :
\[\arg\big\langle \bf{x}(t), \bf{x}(t_0)\big\rangle = \theta = -\Im m\int_{t_0}^t \frac{\big\langle \bf{x}(s) , \bf{x}'(s) \big\rangle}{\|\bf{x}(s)\|^2} ds\]
\skipl

Or, si c'est vrai dans le cas univarié, dès que $n\geq 2$, ce n'est plus le cas. Cette erreur, cette nouvelle phase ne peut pas être 
...
\begin{definition}[valeur principale de Cauchy]\label{def:vp&Hilb}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	Ici $\mathcal{S}(\R)$ est l’espace de Schwartz des fonctions $C^\infty$ à décroissance rapide et la limite en $\varepsilon$ assure que l'intégrale (impropre) converge bien. La valeur de cette intégrale est également appelée sa \emph{valeur principale}. 
\end{definition}
...
Finalement, la condition sur $\SA{x}$ se traduit bien par :
\begin{align*}
	\Fou{\SA{x}} = 2\one_{\R^+}\fou{x} \ \Llr\ \SA{x} &= \iFou{\one_{\R^+}\fou{x}} \\
