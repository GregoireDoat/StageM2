...
Le fait que $\bf{x}$ soit à valeur dans $\C^n$ impose un choix narutelle de d'amplitude instantanée : sa norme. L'on notera alors dans tout la suite (sauf précision) :
\[\forall t\in\R,\qquad 
...
\begin{align}\label{eq:densi_dEi}
	\rho_i\ &:\quad \begin{aligned}\R\ &\lr\quad \R^+ \\ t\ &\longmapsto\ \big|x_i(t)\big|^2 = a(t)^2 a_i(t)^2 \end{aligned}  
	&
...
\begin{align}\label{eq:densi_dE-mv}
	\rho\ &:\quad \begin{aligned}\R\ &\lr\quad \R^+ \\ t\ &\longmapsto\ \big\|\bf{x}(t)\big\|^2 = \sum_{i=1}^n \rho_i(t) \end{aligned}  
	&
	\varrho\ &:\quad \begin{aligned}\R\ &\lr\quad \R^+ \\ \nu\ &\longmapsto\ \big\|\fou{\bf{x}}(\nu)\big\|^2 = \sum_{i=1}^n \varrho_i(t) \end{aligned}	
\end{align}
\end{definition}
...
\begin{align*}
	\esp[\varrho]{\nu} = \int_\R \nu\varrho(\nu)d\nu &= \int_\R \nu\sum_{i=1}^n \varrho_i(\nu) d\nu \\
		 ...
		 \\ &= \frac{1}{2\pi} \esp[\rho]{\sum_{i=1}^n \phi_i'{a_i}^2}
\end{align*}
\\
...
\begin{proposition}[phase dynamique]\label{prop:phased_biva}
	Si on se donne un signal bivarié de la forme (AM--FM--PM) :
	...
	\end{equation}
	\\
	alors, la phase dynamique de $\bf{x}$ est donnée par :
	...
	\\
	Soit une différence de phase $\varphi$ mais avec un terme en plus. Donc $\varphi$ ne doit (\textbf{doit?}) pas être interpréter comme la phase instantanée du signal, où du moins pas au sens donnée dans la \cref{subsec:param_instant_mvar}
\end{proposition}
\begin{demo}%[\cref{prop:phased_biva}]
...
\begin{proposition}[phase totale]\label{prop:phaset_biva}
	La phase totale d'un signal AM--FM--PM est donnée par :
	\begin{equation}
		\phaset(\bf{x},t_0,t) = \arg\big\langle \bf{x}(t_0), \bf{x}(t)\big\rangle = \varphi(t_0)-\varphi(t) + \arg\big(\cos\Delta\theta \cos\Delta\chi - i\sin\Delta\theta \sin(\chi_1+\chi_2)\big)
	\end{equation}
\end{proposition}
\begin{demo}%[\cref{prop:phaset_biva}]
	On note cette fois $\mathcal{V} = \begin{pmatrix} \cos\chi \\ -i\sin\chi \end{pmatrix}$ et sachant la forme des signaux AM--FM--PM, on a :
	\begin{align*}
	...
	\end{align*}
	Pour alléger les notations, on note $\ \Delta y =y(t)-y(t_0)\ $ et $\ y_1=y(t_0), y_2=(t)\ $ pour $\ y=\varphi,\theta,\chi$. Le produit hermitien à droite s'écrit alors :
	\begin{align*}
