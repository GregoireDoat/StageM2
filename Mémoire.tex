...
\subsection{Phase et fréquence instantanée de signal multivarié}
...
\\
Il faut donc une contrainte supplémentaire sur $\phi$. Cette fois-ci, elle ne pourra pas simplement venir des spectres $\fou{x}_i$ puisque, d'une part ils ont déjà été exploités par $\mathcal{A}$. Mais aussi et surtout parce qu'elle doit tenir compte des liens implicites qui existe entre chacune des composantes du signal. \textbf{Est-ce que ca pourrait venir de l'\cref{eq:sum_esp_null} ?}
\\
...
Dans un premier temps, on force la phase $\phi$ --- pour l'instant inconnue --- à apparaitre en écrivant $\bf{x}$ soit la forme :
\[\forall t\in\R,\qquad \bf{x}(t) = e^{i\phi(t)} e^{-i\phi(t)} \bf{x}(t) = e^{i\phi(t)} \Tilde{\bf{x}}(t)\]
\\
Si $\phi$ est bien choisie, alors $\Tilde{\bf{x}}$ contient les informations associées à l'amplitude et la polarisation de $\bf{x}$. Or, il a été vu dans \namecref{sec:temp-freq} précédente qui la phase doit contenir les hautes fréquences du signal. 
\\
...
\[\forall t\in\R,\qquad \varphi(t) = \argmin{\phi(t)}{\|\Tilde{\bf{x}}'(t)\|_2}^2 = \argmin{\phi(t)}{\|e^{-i\phi}\big(\bf{x}' - i\phi'\bf{x}\big)\|_2}^2\]
\\
La contrainte ne dépendant que de $\phi'$, on a de façon équivalente :
\[\min_{\phi(t)}{\|\Tilde{\bf{x}}'(t)\|_2}^2 = \min_{\phi'(t)}{\Big\|e^{-i\phi(t)}\big(\bf{x}'(t) - i\phi'(t) \bf{x}(t)\big) \Big\|_2}^2 = \min_{\phi'(t)}{\big\| \bf{x}'(t) - i\phi'(t) \bf{x}(t) \big\|_2}^2\]
\\
En rappelant que, dans le cas complexe, $\frac{d}{dx}{\big\|f(x)\big\|_2}^2 = 2\Re e\big\langle f(x)\,|\, f'(x)\big\rangle$, il vient que ce minimum est atteint par $\varphi(t)$ à condition que :
\begin{align*}
	0 = \frac{d}{d\varphi'}{\big\| \bf{x}' - i\varphi' \bf{x}\big\|_2}^2 
		&= 2\Re e\left\langle  \bf{x}' - i\varphi' \bf{x} \,\Big|\,  \frac{d}{d\varphi'}\big(\bf{x}' - i\varphi' \bf{x}\big)\right\rangle \\
		&= 2\Re e\big\langle  \bf{x}' - i\varphi' \bf{x} \,|\,  - i \bf{x}\big\rangle \\
		&= 2\Re e\Big(i\big\langle  \bf{x}' \,|\,  \bf{x}\big\rangle\Big) + 2\varphi'\Re e\big\langle   \bf{x} \,|\,  \bf{x}\big\rangle\\
		&= -2\Im m\big\langle  \bf{x}' \,|\,  \bf{x}\big\rangle + 2\varphi'{\| \bf{x}\|_2}^2
\end{align*}
Ce qui mène à poser :
\begin{equation}
	\varphi' = \frac{i\Im m\big\langle  \bf{x}' \,|\,  \bf{x}\big\rangle}{{\| \bf{x}\|_2}^2} = \frac{-i\Im m\big\langle  \bf{x} \,|\,  \bf{x}'\big\rangle}{{\| \bf{x}\|_2}^2}
\end{equation}
Ce qui donne $\varphi$ (à une constante près) :
\begin{equation}
	\forall t\in\R,\qquad \varphi(t) = -\Im m\int_{t_0}^t \frac{\big\langle \bf{x}(s) \,|\, \bf{x}'(s) \big\rangle}{\|\bf{x}(s)\|^2} ds
\end{equation}
...
\begin{itemize}
	...
	\item Quoi qu'il en soit, les cas dim 2 et 3 c'est fait \cite{lilly_wavelet_2006, lilly_bivariate_2010, lefevre_polarization_2021}
\end{itemize}
