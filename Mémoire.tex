...
Dans un premier temps, on force la phase $\phi$ --- pour l'instant inconnue --- à apparaître en écrivant $\bf{x}$ sous la forme :
...
\[\forall t\in\R,\qquad \phi(t) = \argmin{\varphi(t)}{\big\|\Tilde{\bf{x}}'(t)\big\|_2}^2 = \argmin{\varphi(t)}{\Big\|e^{-i\phi}\big(\bf{x}' - i\varphi'\bf{x}\big)\Big\|_2}^2 = \argmin{\varphi(t)}{\big\|\bf{x}' - i\varphi'\bf{x}\big\|_2}^2\]
\\
