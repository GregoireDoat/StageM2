...
\section[labida]{Prérequis mathématique}
...
Les espaces projectifs complexes se construisent ainsi. On se place dans $\C^*$ avec la relation d'équivalence, $\forall x,y\in{\C^n}^*$ :
\[x \sim y\ \Llr\ \exists \lambda\in\C^*\ |\ x=\lambda y\]
\\
L'espace projectif complexe de dimension $n-1$, noté $\PC{n-1}$ est l'espace quotient :
\[\PC{n-1} = {\C^n}^*/\C^* = {\C^n}^*/\sim\]
\\
En notant $[z]$ la classe de $\PC{n-1}$ du représentant $z = (z^i)_{0\leq i\leq n}\in{\C^n}^*$, on définit les ensembles et cartes, $\forall i\in\llbracket0,n\rrbracket$ :
\begin{align}
	U_i &= \Big\{[z]\in\PC{n-1}\ \big|\ z^i\neq 0\Big\}  &  \phi_i\  :\quad &\begin{aligned}
		U_i\ \ &\lr\quad\ \C^{i}\times \{1\} \times\C^{n-i-1}\cong \C^n \\ [z]\ \ &\longmapsto\ \frac{1}{z^i}\big(z_0,\cdots, 1,\cdots, z_n\big)
	\end{aligned}
\end{align}
\\
L'ensemble d'arrivé $\phi_i(U_i)$ est de dimension $n-1$ et s'assimile à $\C^{n-1}$ mais, par souci de comodité, l'on reste dans $\C^n$. Cela permet  d'écrire plus simplement les formules de changement de carte en évitant de devoir rajouter et enlever des coefficients :
...
Les $(U_i,\phi_i)$ forme un atlas holomorphique sur l'espace projectif complexe, faisant de $\PC{n}$ une variété complexe de dimension $n$.
